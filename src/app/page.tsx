...
export default function PublicUserPortalPage() {
  ...
  // Clear & Hide History States
  const [clearedHistoryTimestamp, setClearedHistoryTimestamp] = useState<number>(() => {
    if (typeof window === "undefined") return 0;
    try {
      const stored = localStorage.getItem("portal_cleared_history_ts");
  ...
  const [hiddenHistoryIds, setHiddenHistoryIds] = useState<Set<string>>(() => {
    ...
    try {
      const stored = localStorage.getItem("portal_hidden_history_ids");
  ...
  // Clear & Hide History Handlers
  const handleClearAllHistory = () => {
    const now = Date.now();
    setClearedHistoryTimestamp(now);
    try {
      localStorage.setItem("portal_cleared_history_ts", String(now));
      localStorage.removeItem("hasamitra_my_requests");
    } catch {}
    setClearHistoryModalOpen(false);
    toast.success("Riwayat pengajuan pada perangkat ini berhasil dibersihkan!");
  };

  const handleHideHistoryItem = (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    setHiddenHistoryIds((prev) => {
      const next = new Set(prev);
      next.add(id);
      try {
        localStorage.setItem("portal_hidden_history_ids", JSON.stringify(Array.from(next)));
      } catch {}
      return next;
    });
    toast.success("Item pengajuan dihapus dari riwayat!");
  };
  ...
  const filteredHistory = notifications.filter((item) => {
    if (hiddenHistoryIds.has(item.id)) return false;
    if (clearedHistoryTimestamp > 0) {
      const itemTime = new Date(item.createdAt).getTime();
      if (itemTime <= clearedHistoryTimestamp) return false;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50/80 flex font-sans">
                  ...
                  <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
                    {filteredHistory.length > 0 && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100/80 px-3.5 py-2 rounded-xl transition cursor-pointer border border-rose-200 shadow-2xs"
                        title="Bersihkan daftar riwayat pengajuan di perangkat ini"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-[#FF5500] hover:text-[#e04b00] bg-orange-50/80 hover:bg-orange-100/80 px-3.5 py-2 rounded-xl transition cursor-pointer border border-orange-200/60 shadow-2xs disabled:opacity-60"
                    >
                      <svg
                ...
                <div className="divide-y divide-gray-100">
                  {filteredHistory.length === 0 ? (
                    <div className="p-12 text-center text-slate-400">
                      ...
                      <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                        {historySearch || historyFilterStatus
                          ...
                          : "Daftar riwayat pada perangkat Anda kosong. Silakan isi formulir pengajuan untuk mulai memantau status."}
                      </p>
                      {clearedHistoryTimestamp > 0 && (
                        <button
                          type="button"
                          onClick={() => {
                            setClearedHistoryTimestamp(0);
                            setHiddenHistoryIds(new Set());
                            try {
                              localStorage.removeItem("portal_cleared_history_ts");
                              localStorage.removeItem("portal_hidden_history_ids");
                            } catch {}
                            toast.success("Riwayat pengajuan berhasil dipulihkan!");
                          }}
                          className="mt-3 text-xs font-bold text-[#FF5500] hover:underline cursor-pointer inline-flex items-center gap-1"
                        >
                          <span>↺ Pulihkan Riwayat Sebelumnya</span>
                        </button>
                      )}
                    </div>
                  ) : (
                    filteredHistory.map((item, idx) => (
                      <div
                        key={item.id}
                        className="p-4 sm:p-6 hover:bg-slate-50/80 transition-colors space-y-3 relative group"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2 self-start sm:self-auto shrink-0">
                            ...
                            <button
                              type="button"
                              onClick={(e) => handleHideHistoryItem(item.id, e)}
                              className="p-1 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition cursor-pointer"
                              title="Hapus item ini dari riwayat perangkat"
                            >
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              </svg>
      ...
      <Modal
        ...
        title="Bersihkan Riwayat Pengajuan"
        subtitle="Konfirmasi pembersihan daftar status pada perangkat"
        size="sm"
        footer={
      ...
      >
        <div className="space-y-3 text-xs text-slate-600 leading-relaxed">
          <p>
            Apakah Anda yakin ingin <b>membersihkan seluruh riwayat pengajuan</b> dari tampilan portal di perangkat ini?
          </p>
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-[11px] font-medium flex items-start gap-2">
            ...
            <span>
              Catatan: Data pengajuan resmi tetap tersimpan aman di database server Administrator dan tidak terhapus secara permanen.
            </span>
          </div>
        </div>
      </Modal>
